...
import { BadgeCheck, LineChart, MapPin, Navigation, User } from 'lucide-react';
      ...
      <div className='p-5 pt-8'>
        ...
        <div className='flex flex-wrap justify-between max-w-lg items-center gap-3 my-5'>
          <div className='flex items-center text-sm text-gray-600'>
            ...
            <span className='text-lg font-medium text-slate-800 mr-1.5'>{listing.followers_count?.toLocaleString() || "0"}</span>followers
          </div>
        {
        ...
        <div className='flex items-center justify-between'>
          ...
          <button onClick={()=> {navigate(`/listing/${listing.id}`); window.scrollTo(0,0)}}className='px-7 py-3 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition'>
            More Details
          </button>
        </div>
      </div>
