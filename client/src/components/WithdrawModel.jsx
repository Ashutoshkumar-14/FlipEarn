...
const WithdrawModel = ({ onClose }) => {
    ...
    const handleSubmission = async (e) => {
        e.preventDefault();
        try {
            //check if there is at leaast ane field
            if (account.length === 0) {
            ...
            const { data } = await api.post('/api/listing/withdraw', { account, amount: parseInt(account) }, {
                headers: {
            ...
            <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-auto flex flex-col'>
                ...
                <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
                    <div className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
                        Amount <input onChange={(e) => setAmount(e.target.value)} value={amount} type="nummber" className='w-full px-2 py-1.5 text-sm border border-gray-300 rounded outline-indigo-400' required />
                    </div>
