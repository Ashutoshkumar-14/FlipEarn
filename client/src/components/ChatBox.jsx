...
const ChatBox = () => {
    ...
    const handleSendMessage = async (e)=>{
        ...
        setMessages([...messages, {id: Date.now(),chatId: chat.id, senderId: user.id, message: newMessage, createdAt: new Date()}]);
        setNewMessage("")
    }
        ...
        <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-2xl h-screen sm:h-[600px] flex flex-col'>
            ...
            <div className='bg-gradient-to-r from-indigo-600 to-indigo-400 text-white p-4 sm:rounded-t-lg flex items-center justify-between'>
                <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-indigo-100 trancate'>{user.id === listing?.ownerId ? `Chattinng with buyer(${chat?.chatUser?.name || 'Loading...'})` : `Chatting with seller(${chat?.ownerUser?.name || 'Loading...'})`}</p>
                </div>
                ...
                { isLoading ? (
                    ...
                    messages.map((message)=>(
                        <div key={message.id} className={`flex ${message.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[70%] rounded-lg p-3 ${message.senderId === user.id ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-200 text-gray-800'} shadow-sm`}>
                                <p className='text-sm break-words whitespace-pre-wrap'>{message.message}</p>
                                <p className={`text-[10px] mt-1 ${message.senderId === user.id ? "text-indigo-200" : "text-gray-400"}`}> 
