...
import { platformIcons } from '../utils/platformIcons'; // ✅ FIXED
...
export const MyListings = () => {
  // ✅ FIXED: Added default values and optional chaining
  ...
  const activeListings = userListings?.filter((listing) => listing.status === 'active').length || 0;
  const soldListings = userListings?.filter((listing) => listing.status === 'sold').length || 0; // ✅ FIXED
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      default:
        return <Clock className='size-3.5'></Clock>
    }
  }
  ...
  return (
    <div className='px-6 md:px-16 lg:px-24 xl:px-32 pt-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6 mb-8'>
        {/* ✅ FIXED: Added optional chaining */}
        <StatCard title='Total Listings' value={userListings?.length || 0} icon={<Eye className='size-6 text-indigo-600' />} color='indigo' />
        ...
        <StatCard title='Total Value' value={`${currency}${totalValue.toLocaleString()}`} icon={<DollarSign className='size-6 text-red-600' />} color='red' />
      ...
      <div className='flex flex-col sm:flex-row justify-between gap-4 xl:gap-20 p-6 mb-10 bg-white rounded-xl border border-gray-200'>
        {[
          { label: 'Earned', value: balance?.earned || 0, icon: WalletIcon }, // ✅ FIXED
          { label: 'Withdrawn', value: balance?.withdrawn || 0, icon: ArrowDownCircleIcon }, // ✅ FIXED
          { label: 'Available', value: balance?.available || 0, icon: CoinsIcon }, // ✅ FIXED
        ].map((item, index) => (
                            ...
                            <div className='invisible group-hover:visible absolute right-0 top-0 pt-5 z-10'>
                              <div className='bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3'>
                                ...
                                <button className='text-nowrap'>
                                  Status :{" "}
                                  <span className={
                                    !listing.isCredentialSubmitted ? "text-red-600" :
                                      !listing.isCredentialVerified ? "text-slate-600" :
                                        listing.isCredentialChanged ? "text-green-600" : "text-indigo-600"
                                  }>
                                    {listing.isCredentialSubmitted ?
      ...
      <div className='bg-white border-t border-gray-200 p-4 text-center mt-28'>
        <p className='text-sm text-gray-500'>© 2025 <span className='text-indigo-600'>GreatStack</span>.All rights reserved.
        </p>
      </div>
    </div>
  )
